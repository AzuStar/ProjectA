...
public partial class AudioPlayerSingleton : Node
{
   ...
   [ExportGroup("Assets")]
   [Export]
   public AudioStream _initialMusic;
   ...
   public async void PlayMusic(AudioStream stream, float crossFade = 0.0f)
   {
      try
      {
         ...
         if (_musicPlayer.Playing)
         {
            if (crossFade > 0.0f)
            {
               PropertyTweener? fadeOutTween = CreateTween()
                  .TweenProperty(_musicPlayer, "volume_linear", 0.0f, crossFade);
   ...
   public AudioStreamPlayer3D PlaySfx(AudioStream stream, Vector3 position, float pitchVariance = 0.08f)
   {
      if (_streamQueueSpatial.TryDequeue(out AudioStreamPlayer3D spatial))
      {
         ...
         spatial.PitchScale = 1.0f + (float)GD.RandRange(-pitchVariance, pitchVariance);
